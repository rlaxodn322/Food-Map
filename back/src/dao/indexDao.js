...
exports.selectRestaurants = async function (connection, category) {
  ...
  const selectCategorizedRestaurantsQuery = `SELECT title,address,videoUrl,category FROM Restaurants where status = "A" and category = ?;`;
...
exports.updateStudents = async function (
  ...
  const Query = `update Students set studentName = 
  ifnull(?, studentName), major=ifnull(?, major), birth =ifnull(?, birth), address= ifnull(?, address) where studentIdx = ?;`;
